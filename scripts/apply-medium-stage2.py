...
def apply_rules() -> None:
    path = Path("firestore.secure.rules")
    rules = path.read_text()

    if "function validNotificationPreferences(data)" not in rules:
    ...
    if "function validAvailabilityDay(day)" not in rules:
        rules = replace_section(
            rules,
    ...
    if "function validClinicalCase(data)" not in rules:
        rules = replace_section(
            rules,
    ...
    old_write = """    match /medicalWorkspaces/{bookingId} {
      allow read: if canReadLessonData(bookingId);
      allow create, update: if canWriteLessonData(bookingId)
        && validMedicalWorkspace(request.resource.data, bookingId);
      allow delete: if isOwner();
"""
    new_write = """    match /medicalWorkspaces/{bookingId} {
      allow read: if canReadLessonData(bookingId);
      ...
      allow delete: if isOwner();
"""
    if old_write in rules:
        rules = rules.replace(old_write, new_write, 1)

    path.write_text(rules)


def apply_medical_workspace_client() -> None:
    path = Path("artifacts/medstart/lib/medical-workspace.ts")
    text = path.read_text()

    if "const normalizedCurrent = normalizeWorkspace(bookingId, current)" not in text:
        start = text.index("    transaction.set(\n")
        end = text.index("\n    )", start) + len("\n    )")
        replacement = """    const normalizedCurrent = normalizeWorkspace(bookingId, current)
...
def update_medium_tests() -> None:
    path = Path("scripts/test-medium-rules.mjs")
    text = path.read_text()
    text = text.replace(
        "availabilityDay(true, '18:00', '09:00')",
        "availabilityDay(true, '25:00', '09:00')",
    )
    path.write_text(text)


apply_rules()
apply_medical_workspace_client()
update_medium_tests()
