...
export default function TutorsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .sort((left, right) => {
        ...
        if (sort === 'price-asc') {
          return (left.lessonPrice || Number.POSITIVE_INFINITY) -
  ...
  }, [favoriteIds, favoritesOnly, format, items, maxPrice, profile, sort, specialization, term])
  ...
  const profileNeeds = profile?.subjects?.filter(Boolean).slice(0, 4) ?? []
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-teal-50/85 sm:text-base">
              Сравнивайте дисциплины, формат, стоимость и опыт. Заявка не требует
              оплаты — преподаватель сначала подтверждает время.
            </p>
          </div>
      ...
      {profileNeeds.length > 0 && (
        <section className="rounded-[28px] border border-teal-200 bg-teal-50 p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/dashboard/profile" className="ms-btn ms-btn-secondary shrink-0">
              Изменить запрос
            </Link>
          </div>
        </section>
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(240px,1fr)_repeat(4,minmax(150px,210px))]">
          ...
          <select
            value={format}
            onChange={(event) => setFormat(event.target.value as 'all' | LessonFormat)}
            className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-teal-500"
          >
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-slate-500">
              Найдено: <strong className="text-slate-900">{filtered.length}</strong>
            </span>
            {activeFilters && (
              <button type="button" onClick={resetFilters} className="ms-link-action text-sm">
      ...
      {error ? (
        ...
        <div className="rounded-3xl border border-dashed border-teal-300 bg-teal-50 p-10 text-center">
          ...
          <h2 className="mt-4 text-xl font-black text-slate-950">
            {activeFilters ? 'Ничего не найдено' : 'Преподаватели пока не опубликованы'}
          </h2>
          ...
          {activeFilters && (
            <button type="button" onClick={resetFilters} className="mt-5 ms-btn ms-btn-primary">
              Сбросить фильтры
            </button>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-4 pr-12">
                  {tutor.avatar ? (
                    <ProfilePhoto src={tutor.avatar} size={64} className="h-16 w-16 rounded-2xl object-cover" />
                  ) : (
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate text-lg font-black text-slate-950">{tutor.displayName}</h2>
                ...
                <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm text-slate-500">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {tutor.lessonFormats?.includes('in_person') ? 'Онлайн и очно' : 'Онлайн'}
                  </span>
                </div>
                ...
                {Boolean(tutor.subjects?.length) && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {tutor.subjects!.slice(0, 4).map((subject) => (
                      <span key={subject} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">
                        {subject}
                      </span>
                    ))}
                  </div>
                ...
                <Link href={`/dashboard/tutors/${tutor.uid}`} className="mt-5 ms-btn ms-btn-primary ms-btn-block">
                  Открыть профиль
