    ...
    <div className="space-y-6">
      ...
      {role === 'student' && (
        <section className="grid gap-4 sm:grid-cols-3">
          <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-400">От ваших преподавателей</p>
          </article>
          <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-slate-500">Видео и документы</p>
      ...
      {open && role === 'tutor' && (
        <section className="rounded-[28px] border border-teal-200 bg-white p-5 shadow-lg sm:p-7">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-black text-slate-950">Новый материал</h2>
              <p className="mt-1 text-sm text-slate-500">Материал увидит только выбранный студент.</p>
            </div>
            <button type="button" onClick={() => setOpen(false)} className="ms-icon-btn ms-icon-btn-neutral" aria-label="Закрыть">
              <X className="h-5 w-5" />
            </button>
          </div>
          <form onSubmit={submit} className="mt-6 grid gap-4 lg:grid-cols-2">
            <label className="space-y-2 text-sm font-medium text-slate-700">
              Студент
              <select className={inputClass} value={bookingId} onChange={(event) => setBookingId(event.target.value)} required>
                <option value="">Выберите студента</option>
                {bookingOptions.map((booking) => (
                  <option key={booking.id} value={booking.id}>{booking.studentName}</option>
                ))}
              </select>
            </label>
            <label className="space-y-2 text-sm font-medium text-slate-700">
              Тип
              <select className={inputClass} value={kind} onChange={(event) => setKind(event.target.value as MaterialKind)}>
            ...
            <label className="space-y-2 text-sm font-medium text-slate-700 lg:col-span-2">
              Название
              <input className={inputClass} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Например: Атлас по анатомии" required />
            </label>
            {kind !== 'note' && (
              <label className="space-y-2 text-sm font-medium text-slate-700 lg:col-span-2">
                Ссылка
                <input type="url" className={inputClass} value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://..." required />
              </label>
            )}
            <label className="space-y-2 text-sm font-medium text-slate-700 lg:col-span-2">
              Комментарий
              <textarea className={`${inputClass} min-h-28 resize-y`} value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Что посмотреть или выполнить?" />
            </label>
            <button disabled={saving} className="ms-btn ms-btn-primary disabled:opacity-60 lg:col-span-2 lg:justify-self-end">
              {saving ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <Plus className="h-5 w-5" />}
              Сохранить материал
            </button>
          </form>
        </section>
      ...
      {role === 'student' && materials.length > 0 && (
        <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          ...
          <p className="mt-3 text-sm text-slate-500">Показано: {filteredMaterials.length} из {materials.length}</p>
        </section>
      )}
      ...
      {loading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filteredMaterials.map((material) => {
            ...
            return (
              <article key={material.id} className={`flex flex-col rounded-[28px] border bg-white p-5 shadow-sm ${completed ? 'border-teal-200' : 'border-slate-200'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${completed ? 'bg-teal-50 text-teal-700' : 'bg-sky-50 text-sky-700'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{kindLabels[material.kind]}</span>
                  </div>
                </div>
                <h2 className="mt-5 text-lg font-black text-slate-950">{material.title}</h2>
                <p className="mt-1 text-sm font-bold text-teal-700">{role === 'tutor' ? material.studentName : material.tutorName}</p>
                <p className="mt-3 flex-1 whitespace-pre-wrap text-sm leading-6 text-slate-500">{material.description || 'Без дополнительного комментария.'}</p>
                <div className="mt-5 flex flex-wrap gap-2">
                  {material.url && (
                    <a href={material.url} target="_blank" rel="noreferrer" className="ms-btn ms-btn-primary ms-btn-sm flex-1">
                      Открыть
                      <ExternalLink className="h-4 w-4" />
                    </a>
                  )}
                  {role === 'student' && (
                    <button type="button" onClick={() => toggleCompleted(material.id)} className={`ms-btn ms-btn-sm ${completed ? 'ms-btn-soft' : 'ms-btn-secondary'}`}>
                      {completed ? <CircleCheckBig className="h-4 w-4" /> : <Check className="h-4 w-4" />}
                      {completed ? 'Изучено' : 'Отметить изученным'}
                    </button>
                  )}
                  {role === 'tutor' && (
                    <button type="button" onClick={() => void remove(material)} disabled={deletingId === material.id} className="ms-icon-btn ms-icon-btn-danger" aria-label="Удалить материал">
                      {deletingId === material.id ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                    </button>
                  )}
                </div>
              </article>
        ...
        <div className="rounded-[28px] border border-dashed border-teal-300 bg-teal-50 p-10 text-center">
          ...
          <h2 className="mt-4 text-xl font-black text-slate-950">
            {materials.length ? 'По фильтрам ничего не найдено' : 'Материалов пока нет'}
          </h2>
