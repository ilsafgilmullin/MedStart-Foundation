...
export default function TutorStudentsPage() {
  ...
  const visible = useMemo(() => {
    ...
    const filtered = students.filter((student) => {
      const text =
        `${student.name} ${student.subjects.join(' ')}`.toLocaleLowerCase(
          'ru-RU',
        )
      const matchesFilter =
        filter === 'all' ||
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-teal-50/80 sm:text-base">
              История занятий, текущие дисциплины, материалы и следующий шаг по
              каждому ученику — в одном рабочем пространстве.
            </p>
          </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">
                ...
                особенности объяснения. Заметка хранится только на этом
                устройстве.
              </p>
            </div>
            <button
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px_190px]">
          <label className="relative">
            ...
            <input
              ...
              placeholder="Имя ученика или предмет"
      ...
      {loading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((student) => {
            const completionRate = student.lessonsCount
              ? Math.round(
                  (student.completedCount / student.lessonsCount) * 100,
                )
              : 0
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {student.completedCount} завершено ·{' '}
                      {student.materialsCount} материалов
                    </p>
                  </div>
                  {notes[student.uid] && (
                    <span
                      title="Есть личная заметка"
                      className="rounded-full bg-amber-50 p-2 text-amber-700"
                    >
                ...
                <div className="mt-5 rounded-2xl bg-slate-50 p-4">
                  ...
                  {currentBooking ? (
                    <>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        {formatBookingDate(currentBooking)}
                      </p>
                    </>
                  ) : (
                ...
                {!student.nextLesson && student.completedCount > 0 && (
                  <div className="mt-4 flex items-start gap-2 rounded-2xl border border-amber-100 bg-amber-50 p-3 text-sm text-amber-800">
                    ...
                    <span>
                      Нет следующего занятия — можно предложить продолжение.
                    </span>
                  </div>
                )}
                ...
                <div className="mt-5 grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    className="ms-btn ms-btn-soft ms-btn-sm col-span-2"
                  >
                    <ClipboardPen className="h-4 w-4" />
                    {notes[student.uid]
                      ? 'Открыть заметку'
        ...
        <div className="rounded-[28px] border border-dashed border-teal-300 bg-teal-50 p-10 text-center">
          ...
          <h2 className="mt-4 text-xl font-black text-slate-950">
            {students.length
              ? 'По выбранным условиям учеников нет'
              : 'Учеников пока нет'}
          </h2>
          <p className="mt-2 text-slate-500">
            {students.length
              ? 'Измените поиск или фильтр.'
              : 'После принятия первой заявки ученик появится здесь.'}
          </p>
          {students.length ? (
            <button
              type="button"
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Используйте историю учеников, чтобы планировать повторение и
                выдавать материалы последовательно.
              </p>
            </div>
            <Link
