...
export default function MedicalWorkspace({
  ...
  async function handleUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    if (
      !workspace.privacy.deidentified ||
      !workspace.privacy.identifiersRemoved ||
      !workspace.privacy.consentConfirmed ||
      !workspace.privacy.educationalUseOnly
    ) {
      setActiveModule('privacy')
      setError('Сначала завершите обязательную проверку обезличивания.')
      return
    }
    setUploading(true)
    setError('')
    try {
      const assetId = await uploadMedicalAsset({
        bookingId,
        uploaderUid: userUid,
      ...
      setNotice('Медицинский файл защищённо загружен в занятие.')
    } catch (caught) {
      setError(
          ...
          {moduleItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-semibold transition sm:px-4 ${
              ...
              <aside className="space-y-4">
                <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                  ...
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/jpeg,image/png,image/webp,.dcm,application/dicom"
                    className="hidden"
                  ...
                  <button
                    ...
                    className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-3 text-sm font-semibold disabled:opacity-60"
                  >
                    {uploading ? (
                  ...
                  <p className="mt-3 text-xs leading-5 text-slate-500">
                    Поддерживаются JPG, PNG, WebP и DICOM. Перед загрузкой
                    обязательна проверка обезличивания.
                  </p>
                </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          {(participantRole === 'tutor' ||
                            ...
                            <span
                              ...
                              onKeyDown={(event) => {
                                if (event.key === 'Enter') void removeAsset(asset)
                              }}
